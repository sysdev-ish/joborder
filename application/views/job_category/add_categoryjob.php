...
<script type="text/javascript">
	$(document).ready(function(){
		$("#btn_simpan").click(function(){
			$('#overlay').show();
			var ljobcategory	 		= $('#ljobcategory').val();
			var url 			= "<?php echo base_url(); ?>index.php/login/jobcategory_simpan/";
			arrjobcategory			= [ljobcategory];
            console.log(arrjobcategory)
			
			$.post(url, {xjobcategory:arrjobcategory}, function(response){
				
					//custom_alert(response,"Informasi");
					alert('Data berhasil di simpan');
					xTable.fnDestroy();
					$('#list_jobcategory tbody').html(response);
					$('#list_jobcategory').dataTable({"bFilter": false, "bSort": true, "bAutoWidth": false, "bLengthChange": false, "bPaginate": true, "scrollX": true});
					$('#ljobcategory').val("");			
		...
		$("#btn_save").click(function(){
			$('#overlay').show();
			var ijobcategory			= $('#ijobcategory').val();
			var iid		 		= $('#iid').val();
			var url 			= "<?php echo base_url(); ?>index.php/login/jobcategory_edit/";
			arrjobcategory				= [iid, ijobcategory];
		...
		var xTable = $('#list_jobcategory').dataTable({
			"bRetrieve": true,
			"bServerside": true,
			"bProcessing": true,
			"bPaginate": true,
		...
		xTable.on( 'draw.dt', function () {
			$(".btn").click(function(){
				...
				var tjobcategory	 = $row.find(".tjobcategory").text();
			
				var tid			= $row.find(".tid").text();
		
				
				$("#ijobcategory").val(tjobcategory);
				$("#iid").val(tid);
			});
		});		
		...
		<div class="col-md-6">
			...
			<div class="box box-primary">
				<div class="box-header with-border">
					<h3 class="box-title">Form Create Job Category</h3>
				</div><!-- /.box-header -->
				<!-- form start -->
				<form role="form">
				<div class="box-body">
					<div class="form-group">
						...
						<div class="input-group">
							...
							<input type="text" class="form-control pull-right" id="ljobcategory">
						</div><!-- /.input group -->
					</div><!-- /.form group -->
			...
			<div class="box box-primary">
				...
				<!-- form start -->
							...
							<div class="input-group">
								...
								<input type="text" class="form-control pull-right" id="search" placeholder="Search Job Category.." onkeypress="handle(event)"/>
							</div>
				
				<form class="form-horizontal">
				<div class="box-body">
					<table id="list_jobcategory" class="table table-bordered table-hover">
						<thead>
							<tr>
								<th>Id</th>
								<th>Job category</th>
		...
		<div class="modal fade" id="myModal">
		  <div class="modal-dialog modal-info" id="modal-alert">
			 <div class="modal-content">
				...
				<div class="form-group">
					<label for="payrollarea" class="col-sm-4 control-label">Category job : </label>
					<div class="input-group">
						<div class="input-group-addon">
							<i class="fa fa-calendar"></i>
						</div>
						<input type="hidden" class="form-control" id="iid">
						<input type="text" class="form-control" id="ijobcategory">
					</div><!-- /.input group -->
				</div><!-- /.form group -->
