...
function approve()
{
	...
	var reponses = "";
	//alert("here");
	var jonoStr = document.getElementById("jonoDetail2").value;

	//alert(jonoStr);
	$.ajax(
		 {
				 type: 'POST',
				 url: '<?php echo base_url(); ?>index.php/JobOrderEvent/JSJOEventOperationApprove',
				 dataType: 'json',
				 data: {'jonos': jonoStr },
				 ...
				 success: function (data) {
					 //alert(" | " +  data.result);
					 getList();
					 $('#myModal3').modal('hide'); $('#myModal').modal('hide');
				 },
...
function reject()
{
	...
	//alert("here");
	var jonoStr = document.getElementById("jonoDetail2").value;
	var commentarys = document.getElementById("rejectsDetail").value;

	//alert(jonoStr);
	if(commentarys.length<5)
	{
			alert("Commentary is mandatory for reject process. Minimum 5 character.");
	}
	else {
		$.ajax(
			 {
					 type: 'POST',
					 url: '<?php echo base_url(); ?>index.php/JobOrderEvent/JSJOEventOperationReject',
					 dataType: 'json',
					 data: {'jonos': jonoStr,'commentary':commentarys },
					 beforeSend: function (jqXHR, settings) {
					 },
					 timeout: 5000,
					 success: function (data) {
...
function rfpDetail(datas)
{
		...
		$.ajax(
			 {
					 type: 'POST',
					 url: '<?php echo base_url(); ?>index.php/JobOrderEvent/JSjobEventOperationRFPShow',
					 dataType: 'json',
					 data: {'jonos': jonoStr },
					 beforeSend: function (jqXHR, settings) {
					 },
					 ...
					 success: function (data) {
						 //alert(" | " +  data.result);
						 var buttonStr = "<button type='button' class='btn btn-danger pull-left' data-dismiss='modal' id='dpn_cancel'><i class='fa fa-close'></i> Cancel</button> ";
						 buttonStr = buttonStr + "<button type='button' class='btn btn-success pull-right' id='dpn_cancel' onclick='approve()'>Approve <i class='fa fa-check-circle-o'></i> </button> ";
		 			 buttonStr = buttonStr + "<button type='button' class='btn btn-warning pull-right' id='dpn_cancel' onclick='reject()'><i class='fa fa-close'></i> Reject  </button> ";
			...
			$.ajax(
				 {
						 ...
						 success: function (data) {
							 //alert(data.result);
								getList();
								$('#myModal').modal('hide');$('#myModal3').modal('hide');
...
function getList()
{
	var criteriaData = document.getElementById("criteriaData").value;
		...
		$.ajax(
			 {
					 type: 'POST',
					 url: '<?php echo base_url(); ?>index.php/JobOrderEvent/JSGetEventOperationApproveList',
					 dataType: 'json',
					// contentType: "application/json; charset=utf-8",
