...
<script src="<?php echo base_url(); ?>public/plugins/big.js" type="text/javascript"></script>
...
<script type="text/javascript">
	$(document).ready(function() {
		$('.check_pks').click(function() {
              $('.check_pks').not(this).prop('checked', false);
         });
		$(".select2").select2();
		...
		$('#sdateskem').datepicker({
			format: 'yyyy-mm-dd',
			autoclose: true
		});
		$('#edateskem').datepicker({
			format: 'yyyy-mm-dd',
			autoclose: true
		});
		$('#tgl1').datepicker({
		...
		$("#addmorekomp").click(function() {
			...
			$("<hr><div class='form-group' id=" + iddivkom + "><label class='col-sm-3 control-label'>Komponen</label><div class='col-sm-9'><select name=" + idkom + " id=" + idkom + " class='kompx form-control select2' style='width:400px;' onchange='getsifat(this," + ckomp + ")'><option value=''></option> <?php //echo $cmbkomp;
		...
		$("#xpernerx").change(function() {
			...
			$.post(url, {
				...
				if (res[0] == 'GAGAL') {
					if (res[1] == '1') {
						alert('Perner ini sudah diresignkan pada nojo ' + res[2] + ', dan sudah di proses !!');
						document.getElementById("xpernerx").disabled = false;
						$("#xpernerx").select2("val", "");
						return false;
					} else {
						alert('Data sudah pernah di input pada nojo ' + res[2] + ', dan belum di proses !!');
						document.getElementById("xpernerx").disabled = false;
						$("#xpernerx").select2("val", "");
						return false;
					}
		...
		*/
		...
		$('body').on('click', '#btn_submit1', function(e) {
			...
			*/
			if (ncust_id == "") {
				...
				$('#overlay1').hide();
				$('#ffgg').show();
				$('#sdivnccust').attr('class', 'form-group has-error');
				return false;
			} else if (ncust_id != "") {
				$('#sdivnccust').attr('class', 'form-group')
			}
		...
		$('body').on('click', '#btn_simpan', function(e) {
			...
			if(tnew == 1){
				if(doc_legalitas == "" ){
					alert('Permintaan Legalitas harus dipilih');
					$('#overlay').hide();
					$('#ggff').show();
					return false;
				}
			...
			// if(pks == "undefined" ){
			// 	alert('Kebutuhan PKS harus dipilih');
			// 	$('#overlay').hide();
			// 	$('#ggff').show();
			// 	return false;
			// }
			...
			if (komponen5 == "") {
				...
				$('#overlay').hide();
				$('#ggff').show();
				$('#group_att4').attr('class', 'form-group has-error');
				return false;
			} else if (komponen5 != "") {
				$('#group_att4').attr('class', 'form-group')
			}
			...
			*/
			...
			$('#listrincian tbody tr').each(function() {
				...
				lrincian += [ljabatan, lgender, lpendidikan, llokasi, lwaktu, latasan, lkontrak, ljumlah, lskema, lskema_text, lskillx, lskillx_text, n_ljabatan, n_llokasi, kump, tot, kidx, lktrain, nlatihan, nbekerja, typeng, lpnorek, ljskema, lreasonrot, ljpkwt];
			...
			$('#listkomponen tbody tr').each(function() {
				...
				lkomponen += [kjabatan, kkodejab, klokasi, kkodelok, kkomponen, kkodekomponen, kvalue, kket, khk, kump, kper, lskema, lskema_text, lskillx, lskillx_text, kidx, kpengtk, kpengkes];
				lkomponen += ',';
			});
			...
			if (lkomponen == "") {
				...
				$('#listkomponen tbody tr').each(function() {
					...
					var kpengkes = $(this).find(".kpengkes").html();
					//lkomponen += [kjabatan, kkodejab, klokasi, kkodelok, kkomponen, kkodekomponen, kvalue, kket, klevel,kkodelevel, khk, kump];
					lkomponenx += [kjabatan, kkodejab, klokasi, kkodelok, kkomponen, kkodekomponen, kvalue, kket, khk, kump, kper, lskema, lskema_text, lskillx, lskillx_text, kidx, kpengtk, kpengkes];
					lkomponenx += ',';
				});
			}
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (type_jo == 2) {
						...
						var type = "POST";
						var mimeType = "multipart/form-data";
						arrjoborder = [tanggal, lama, '', '', komponen4, komponen5, komponen6, type_jo, jenisproject, xtype_jo, typere, xnobak, lperner, jeniscapt, tperal, ncust_id, ncust_name, lpernergan, doc_legalitas];
						$.post(url, {
						...
						var url = "<?php echo base_url(); ?>index.php/rotasi/rincian_simpan_buattes/";
						var type = "POST";
						var mimeType = "multipart/form-data";
						var sapi = vkumdocx;
						arrjoborder = [tanggal, project, syarat, deskripsi, lama, '', '', komponen4, komponen5, komponen6, type_jo, jenisproject, lrincian, xtype_jo, typere, respa, tnew, lkomponenx, xnobak, lproczx, vkumdocx, tglgaji, tpeng, ncust_id, ncust_name, jeniscapt, tperal, sproject, eproject, nilpro, doc_legalitas];
						$.post(url, {
							joborder: arrjoborder,
							sapi: sapi
						}, function(resp) {
		...
		*/
		...
		$('#tmbhrincian').click(function() {
			...
			if (tejo == '2') {
				if (typere == "") {
					alert('Type Replacement tidak boleh kosong');
					$('#divtre').attr('class', 'form-group has-error');
					return false;
				} else if (typere != "") {
					$('#divtre').attr('class', 'form-group')
				}
			}
		...
		*/
		...
		$('#addrincian').click(function() {
			...
			if (jumlah == "") {
				...
				$('#divjumlah').attr('class', 'form-group has-error');
				return false;
			} else if (jumlah != "") {
				$('#divjumlah').attr('class', 'form-group')
			}

			if (lskema == "") {
				alert('Level tidak boleh kosong');
				$('#divlevel').attr('class', 'form-group has-error');
				return false;
			} else if (lskema != "") {
				$('#divlevel').attr('class', 'form-group')
			}

			if (lskillx == "") {
				alert('Skilllayanan tidak boleh kosong');
				$('#divskil').attr('class', 'form-group has-error');
				return false;
			} else if (lskillx != "") {
				$('#divskil').attr('class', 'form-group')
			}
			...
			if (xbekerja == "") {
				...
				$('#overlay').hide();
				$('#ggff').show();
				$('#divbekerjan').attr('class', 'form-group has-error');
				return false;
			} else if (xbekerja != "") {
				$('#divbekerja').attr('class', 'form-group')
			}
			...
			if (latihan == "") {
				...
				$('#overlay').hide();
				$('#ggff').show();
				$('#divlatihann').attr('class', 'form-group has-error');
				return false;
			} else if (latihan != "") {
				$('#divlatihan').attr('class', 'form-group')
			}

			if (typepeng == "3") {
				if (pnorek == "") {
					alert('Perner No REKRUT Harus di Isi untuk Report, Terima Kasih');
					$('#divpnorek').attr('class', 'form-group has-error');
					return false;
				} else if (pnorek != "") {
					$('#divpnorek').attr('class', 'form-group')
				}

				if (alasanrot == "") {
					alert('Alasan Harus Di Isi untuk repot, TerimaKasih');
					$('#divalasanrot').attr('class', 'form-group has-error');
					return false;
				} else if (alasanrot != "") {
					$('#divalasanrot').attr('class', 'form-group')
				}
			}
			...
			// $("#tk_valuex1").prop("checked", true);
			// $("#kes_valuex1").prop('checked', true);
		...
		$('#addkomponen').click(function() {
			...
			if (tperal == 'INF') {
				if (jskema == 1) {
					if (tot > kump) {} else if (tot = kump) {} else {
			...
			if (jskema == 1) {
				...
				for (x = 1; x <= ckomp; x++) {
					...
					if (kkodekomponen != '') {
						$('#listkomponen> tbody:last-child').append('<tr><td class=kidx>' + xmid + '</td><td class=kjabatan>' + kjabatan + '</td><td class=kkodejab style=display:none>' + kkodejab + '</td><td class=klokasi>' + klokasi + '</td><td class=kkodelok style=display:none>' + kkodelok + '</td><td class=lskema_text>' + lskema_text + '</td><td class=lskema style=display:none>' + lskema + '</td><td class=lskillx_text>' + lskillx_text + '</td><td class=lskillx style=display:none>' + lskillx + '</td><td class=kkomponen>' + kkomponen + '</td><td class=kkodekomponen style=display:none>' + kkodekomponen + '</td>' + '</td><td class=kvalue>' + kvaluexy + '</td><td class=kket>' + kket + '</td><td class=kper>-</td><td class=kpengtk>Y</td><td class=kpengkes>Y</td><td class=khk style=display:none>' + khk + '</td><td class=kump style=display:none>' + kump + '</td></tr>');
					}
					...
					tot_fixed += parseFloat(te);
					// tot_fixed += parseFloat(kvalue);
				}
				if (isNaN(tot_fixed)) {
					var fixtot = 0;
				} else {
					var fixtot = tot_fixed;
				}

				console.log('fixed', fixtot);

				var vkomplength = $('.vkompx');
				var vckomp = vkomplength.length;
				var tot_variable = 0;
				//validasi komponen variable
				var x = 1;
				for (x = 1; x <= vckomp; x++) {
					...
					if (vkkodekomponen != '') {
						$('#listkomponen> tbody:last-child').append('<tr><td class=kidx>' + xmid + '</td><td class=kjabatan>' + kjabatan + '</td><td class=kkodejab style=display:none>' + kkodejab + '</td><td class=klokasi>' + klokasi + '</td><td class=kkodelok style=display:none>' + kkodelok + '</td><td class=lskema_text>' + lskema_text + '</td><td class=lskema style=display:none>' + lskema + '</td><td class=lskillx_text>' + lskillx_text + '</td><td class=lskillx style=display:none>' + lskillx + '</td><td class=kkomponen>' + vkkomponen + '</td><td class=kkodekomponen style=display:none>' + vkkodekomponen + '</td>' + '</td><td class=kvalue>' + vkvaluex + '</td><td class=kket>' + vkket + '</td><td class=kper>-</td><td class=kpengtk>' + valpeng_tk + '</td><td class=kpengkes>' + valpeng_kes + '</td><td class=khk style=display:none>' + khk + '</td><td class=kump style=display:none>' + kump + '</td></tr>');
					}
					...
					tot_variable += parseFloat(tee);
				}

				if (isNaN(tot_variable)) {
					var fixvar = 0;
				} else {
					var fixvar = tot_variable;
				}

				console.log('variable', fixvar);

				var kodekont = $("#kodekont").val();
				var totkabeh = 0;
				var totkabeh = fixtot + fixvar;
				console.log('totalkabeh', totkabeh);

				//addby kaha 17-10-2022 kodekont -> type kontrak cek m_kontrak jika 2 (Kemitraan) & 3 (Magang) & 6 (THL) ->abaikan
				if (kodekont == 2 || kodekont == 3 || kodekont == 5) {

				} else {
					if (totkabeh < kump) {
						alert('Total fixed dan variabel harus sama atau lebih dari UMP');
						return false;
					}
				}
				...
				for (x = 1; x <= bckomp; x++) {
					...
					if (vkkodekomponen != '') {
						...
						// }
						// else {
						$('#listkomponen> tbody:last-child').append('<tr><td class=kidx>' + xmid + '</td><td class=kjabatan>' + kjabatan + '</td><td class=kkodejab style=display:none>' + kkodejab + '</td><td class=klokasi>' + klokasi + '</td><td class=kkodelok style=display:none>' + kkodelok + '</td><td class=lskema_text>' + lskema_text + '</td><td class=lskema style=display:none>' + lskema + '</td><td class=lskillx_text>' + lskillx_text + '</td><td class=lskillx style=display:none>' + lskillx + '</td><td class=kkomponen>' + vkkomponen + '</td><td class=kkodekomponen style=display:none>' + vkkodekomponen + '</td>' + '</td><td class=kvalue>' + vkvaluexy + '</td><td class=kket>' + vkket + '</td><td class=kper>-</td><td class=kpengtk>-</td><td class=kpengkes>-</td><td class=khk style=display:none>' + khk + '</td><td class=kump style=display:none>' + kump + '</td></tr>');
						// }
					}
				}
				...
				for (x = 1; x <= chukz; x++) {
					...
					$('#listkomponen> tbody:last-child').append('<tr><td class=kidx>' + xmid + '</td><td class=kjabatan>' + kjabatan + '</td><td class=kkodejab style=display:none>' + kkodejab + '</td><td class=klokasi>' + klokasi + '</td><td class=kkodelok style=display:none>' + kkodelok + '</td><td class=lskema_text>' + lskema_text + '</td><td class=lskema style=display:none>' + lskema + '</td><td class=lskillx_text>' + lskillx_text + '</td><td class=lskillx style=display:none>' + lskillx + '</td><td class=kkomponen>' + anketb + '</td><td class=kkodekomponen style=display:none>' + akketb + '</td>' + '</td><td class=kvalue>' + avbtkpxy + '</td><td class=kket>' + abketb + '</td><td class=kper>' + apbtkp + '</td><td class=kpengtk>-</td><td class=kpengkes>-</td><td class=khk style=display:none>' + khk + '</td><td class=kump style=display:none>' + kump + '</td></tr>');
				}
			}

			var pnorekx = pnorek.replace(",", ";");

			$('#listrincian > tbody:last-child').append('<tr><td class=kidx>' + xmid + '</td><td class=ljabatan>' + jabatan + '</td><td class=lgender>' + gender + '</td><td class=lpendidikan>' + pendidikan + '</td><td class=llokasi>' + lokasi + '</td><td class=lwaktu>' + waktu + '</td><td class=latasan>' + atasan + '</td><td class=lkontrak>' + n_kontrak + '</td><td class=lnjpkwt>' + n_jpkwt_asli + '</td><td class=ljumlah>' + jumlah + '</td><td class=lskema_text>' + lskema_text + '</td><td class=lskema style=display:none>' + lskema + '</td><td class=khk>' + khk + '</td><td class=lskillx_text>' + lskillx_text + '</td><td class=lskillx style=display:none>' + lskillx + '</td><td class=llokasiz style=display:none>' + lokasiz + '</td><td class=ljabatanz style=display:none>' + jabatanz + '</td><td class=kump style=display:none>' + kump + '</td><td class=tot style=display:none>' + tot + '</td> <td class=lktrain_text>' + vkumtraintx + '</td><td class=lktrain style=display:none>' + vkumtrainx + '</td> <td class=typeng style=display:none>' + typepeng + '</td> <td class=ljskema style=display:none>' + jskema + '</td> <td class=ljpkwt style=display:none>' + jpkwt_asli + '</td> <td class=nlatihan>' + latihan + '</td> <td class=nbekerja>' + bekerja + '</td> <td class=ntypeng>' + ntypepeng + '</td> <td class=lpnorek>' + pnorekx + '</td> <td class=lreasonrot>' + alasanrot + '</td> </tr>');
		...
		*/
		...
		$("#addmorekompx").click(function() {
			...
			//$('#tkomp > tbody:last-child').append("<tr><td><select name="+idkom+" id="+idkom+" class='kompx form-control select2' onchange='getsifat(this,"+ckomp+",1)' style='width:250px'><option value=''></option><?php //echo $cmbkomp; 
																																																																																																									?></select></td><td><input type='text' class='form-control pull-right' id="+idval+" name='valuex' style='width:150px'></td><td><select class='form-control pull-right' id="+idket+" style='width:150px'><option value=''>Pilih</option></select></td><td><center><input type='checkbox' class='tk' id="+idcekbpjs_tk+" onclick='jumlahkan_tk(\""+idval+"\",\""+idcekbpjs_tk+"\",\""+idcekbpjs_kes+"\")' name='cekbpjs_tk' value="+idval+" checked></center></td><td><center><input type='checkbox' class='kes' id="+idcekbpjs_kes+" onclick='jumlahkan_kes(\""+idval+"\",\""+idcekbpjs_kes+"\",\""+idcekbpjs_tk+"\")' name='cekbpjs_kes' value="+idval+" checked></center></td>	</tr>");
	...
	*/
	...
	function cari_pkwt(value) {
		if (value == 1 || value == 4 || value == 5 || value == 6) {
			...
			var url = "<?php echo base_url(); ?>index.php/home/pilih_jpkwt";
			$.post(url, {
				data: value
			}, function(response) {
				$("#jpkwt").html(response);
...
-->
						...
						<div class="form-group" id="divproject">
							...
							<div class="form-group hidden" id="divproject">
								<label class="col-sm-3 control-label">Jenis Project</label>
								<div class="col-sm-9">
									<input type="text" class="form-control pull-right" id="jenisproject" name="jenisproject" value="1" readonly=readonly />
							...
							<div class="form-group hidden" id="divcapt">
								<label class="col-sm-3 control-label">Jenis Captive</label>
								<div class="col-sm-9">
									<input type="text" class="form-control pull-right" id="jeniscapt" name="jeniscapt" value="1" readonly=readonly />
						...
						-->
								...
								<div class="form-group">
									...
									<div class="col-sm-9">
										<!-- <input type="checkbox" class="check_pks" name="doc_pks" value="1" id="doc_pks"> Ya <br>
										<input type="checkbox" class="check_pks" name="doc_pks" value="0" id="doc_pks"> Tidak -->
									...
									<div class="box-body">
										...
										<div class="form-group" id="divjabatan">
											...
											<div class="col-sm-9">
												...
												<select class="form-control pull-right select2" id="jabatan" style="width: 100%">
													<option value="">Pilih Jabatan</option>
													<?php echo $cmbdetailkategori; ?>
												</select>
											</div><!-- /.input group -->
										</div><!-- /.form group -->
										...
										<div class="form-group" id="divcategory">
											...
											<div class="col-sm-9">
												...
												<select class="form-control pull-right select2" id="kategori" style="width: 100%">
										...
										<div class="form-group" id="divjpkwt" style="display:none">
											<label class="col-sm-3 control-label">Jenis Kompensasi</label>
											<div class="col-sm-9">
									...
									<div class="form-group" id="divjskema">
										...
										<div class="col-sm-9">
											<select class="form-control pull-right" id="jskema">
												...
												<!-- <option value="3">NEW (MAGANG)</option> -->
											</select>
										</div>
									</div>
														...
														<table id="tkomp_ben" class="table table-bordered table-hover" style="width:850px;">
															...
															<tbody>
																...
																<tr>
																	...
																	<td id="haha">
																		<input type="text" class="form-control" id="pbtkp3" name="ppbtkp" style="width:50px" value="5" onkeypress="return isNumberKey(event,this.id)">% &nbsp;<input type="text" class="form-control" id="vbtkp3" name="vvbtkp" style="width:182px" readonly=readonly>
																	</td>
																...
																<tr>
																	...
																	<td id="haha">
																		<input type="text" class="form-control" id="pbtkp4" name="ppbtkp" style="width:50px" value="0" onkeypress="return isNumberKey(event,this.id)">% &nbsp;<input type="text" class="form-control" id="vbtkp4" name="vvbtkp" style="width:182px" readonly=readonly>
																	</td>
