	...
	$(function () {
		...
		$(".btndok").click(function(){
			...
			$.post(url, {data : vnojo}, function(res){
				...
				$('#listdokumen').dataTable({"bRetrieve": true,"bServerside": true,"bProcessing": true,"bPaginate": true,"bLengthChange": false,"bFilter": false,"bSort": true,"bInfo": true,"bAutoWidth": true,"scrollX": true,"bJQueryUI": false});
			})
		});
		...
		$(".btndetail").click(function(){
			...
			*/	
			...
			$.post(url, {data : vnojo}, function(res){
				...
				$('#listrincian').dataTable({"bFilter": false, "bSort": true, "bAutoWidth": false, "bLengthChange": false, "bPaginate": true, "scrollX": true});
			})
		...
		$("#typeapp").change(function(){
			  ...
			  */
			  ...
			  if (ish == 'VARIABEL'){
				...
				var url = "<?php echo base_url(); ?>" + "index.php/home/appjo_xlistjoxpm/";
				var dataarr = $('#xsearch').val();
				$.post(url, {dataarr:dataarr}, function(response){
					vTable.fnDestroy(); 
					$('#overlayx').hide();
		...
		$(".btnapp").click(function(){
			...
			if (btn == "Approved"){
				...
				$("#labeljoz").html('<label class="control-label"> Anda akan menyetujui Penyesuaian Skema '+ tejo +' </label><br><textarea class="form-control" rows="3" id="ket_app" name="ket_app" placeholder="Alasan Reject..."/></textarea>');
		...
		$('#btnskip').click(function(){
			...
			//arrappjo = [vnojo, ket, ssalary, esalary, exper, level, job_skills, job_benefits, duedate, duedate1];
			$.post(url, {arrrinx : arrrinx}, function(res){
				//alert(res);
...
<script>
function baddx(nojo, jbtn, ntype, ntrep, alasan_atasan){
	//alert('AAA');
	console.log(jbtn);	
	...
	var url	= "<?php echo base_url();?>index.php/home/det_per";
	$.post(url, {data : nojo}, function(res){
		sTable.fnDestroy();
		$('#overlay').hide();
		$('#listperner tbody').html(res);
...
function baddy(nojo, jbtn, ntype, ntrep, alasan_atasan){
	var vnojo = 'a';
	var vjab = 'b';
	var vlok = 'c';
	larr 		 = [vnojo, vjab, vlok];
	...
	$.post(url, {data : nojo}, function(res){
		...
		$('#overlay').hide();
		$('#listdokumen tbody').html(res);
		$('#listdokumen').dataTable({"bFilter": false, "bSort": true, "bLengthChange": false});
	})
						...
						<div class="form-group">
							<div class="input-group">
								...
								<select class="form-control pull-right" id="typeapp" name="typeapp" style="width:400px;"/>
									...
									<option value="VARIABEL">Penyesuaian Variabel</option>										
								</select>	
								</div>
							</div><!-- /.input group -->
						</div><!-- /.form group -->
							...
							<div class="box-group" id="accordion">
								...
								<div class="panel box box-primary">
								  ...
								  <div id="collapseOne" class="panel-collapse collapse in">
									<div class="box-body">
										<table id="listrincian" class="table table-bordered table-hover"  style="width:800px;">
				...
				<div class="modal fade" id="VXmyModal" role="dialog">
				  <div class="modal-dialog" id="modal-alert" style="width:930px;">
					 <div class="modal-content">
						...
						<div class="modal-body">
							<table id="listperner" class="table table-bordered table-hover"  style="width:900px;">
								<thead style="background-color:blue; color:white;">
									<tr>
										<th >Perner</th>
										<th >Nama</th>
...
$(function(){
	...
	$("#listpola").DataTable({
		...
		bSort : true,
		ajax: {
		  url 		: "<?php echo base_url().'index.php/mapping/data_listappjopm';?>",
		  type 		:'POST',
		  dataType	: "json",
		  data		: dataString
		}, 
		...
		columnDefs: [
        { 
            "targets": [ 0,9,10,11,12,13 ], //first column / numbering column
	...
	$("#nbtn_simpan").click(function(){
		...
		var tjen  	= 'New'; 
		$("#inojo").val(vnojo);
		var url	= "<?php echo base_url();?>index.php/home/simpantjo_pm";
