	...
	$(document).ready(function(){
					...
					if(res[0]=='GAGAL'){
						...
						$.ajax({
							...
							success	: function(res){
								...
								var tfgk = res.cname;
								if(tfgk=='' || tfgk==null){
									alert('Perner ini belum di rotasi / mutasi');
									return false;
								} else {
									var anjk = tfgk.replace(",", ".");
									$('#listperner> tbody:last-child').append('<tr><td class=ntres>'+tresign+'</td><td class=ntrr>'+ntrotres+'</td><td class=nperner>'+res.PERNR+'</td><td class=nnama>'+anjk+'</td><td class=narea>'+res.btrtx+'</td><td class=npersa>'+res.wktxt+'</td><td class=nskill>'+res.pektx+'</td><td class=njabatan>'+res.platx+'</td>'+'</td><td class=nlevel>'+res.trfar_txt+'</td><td class=ktrep_train>'+vkumtraintx+'</td><td><button class="btn btn-box-tool btnDelete"><i class="glyphicon glyphicon-trash"></i></button></td><td class=karea style=display:none>'+res.btrtl+'</td><td class=kpersa style=display:none>'+res.werks+'</td><td class=kskill style=display:none>'+res.persk+'</td><td class=klevel style=display:none>'+res.trfar+'</td><td class=kjabatan style=display:none>'+res.stell+'</td><td class=kabkrs style=display:none>'+res.abkrs+'</td><td class=kkrep_train style=display:none>'+vkumtrainx+'</td><td class=krr style=display:none>'+trotres+'</td></tr>');	
								}
							}
						})
					}
		...
		$('body').on('click', '#btn_simpan', function(e){
				...
				*/
				...
				$('#listrincian tbody tr').each(function(){
					...
					lrincian += [ljabatan, lgender, lpendidikan, llokasi, lwaktu, latasan, lkontrak, ljumlah, lskema, lskema_text, lskillx, lskillx_text, n_ljabatan, n_llokasi, kump, tot, kidx, lktrain, ljskema];
				...
				$('#listperner tbody tr').each(function(){
					...
					//lkomponen += [kjabatan, kkodejab, klokasi, kkodelok, kkomponen, kkodekomponen, kvalue, kket, klevel,kkodelevel, khk, kump];
					lperner += [nperner, nnama, narea, npersa, nskill, njabatan, nlevel, karea, kpersa, kskill, klevel, ntres, kkrep_train, ntrr, krr, kjabatan, kabkrs];
					lperner += ',';
				});
				...
				$.ajax({
					...
					success: function (data) {   
							...
							if(type_jo==2){
								...
								var url = "<?php echo base_url(); ?>index.php/home/perner_simpan/";
								var	type 		= "POST";
								var mimeType    = "multipart/form-data";
								arrjoborder = [tanggal, lama, latihan, bekerja, komponen4, komponen5, komponen6, type_jo, jenisproject, xtype_jo, typere, xnobak, lperner, jeniscapt, tperal, ncust_id, ncust_name, pnorek];
								$.post(url, {joborder:arrjoborder}, function(resp){
									$('#overlay').hide();
									alert('Data Tersimpan');
									location.reload();
								...
								var mimeType    = "multipart/form-data";
								var sapi = vkumdocx; 
								arrjoborder = [tanggal, project, syarat, deskripsi, lama, latihan, bekerja, komponen4, komponen5, komponen6, type_jo, jenisproject, lrincian, xtype_jo, typere, respa, tnew, lkomponenx, xnobak, lproczx, vkumdocx, tglgaji, tpeng, ncust_id, ncust_name, jeniscapt, tperal, pnorek];
		...
		*/
		...
		$("#typepeng").change(function(){
			...
			if(typepeng==2){
				$("#bekerja").val('<?php echo $tgbekerja; ?>');
				if (tanggal > latihan){
...
function ValidateSize(file) {
	...
	// if (FileSize > 5) {
		// alert('File size tidak lebih dari 5 MB');
	   // $(file).val(''); 
	// } else {

	// }
	
	// var oFile1 = document.getElementById("komponen1").files[0]; 
...
-->
						...
						<div class="form-group" id="group_att4">
							<label class="col-sm-3 control-label"><?php if($jenis=='1'){echo "Dokumen BAK";}else{echo "MODIN";} ?></label>
							<div class="col-sm-9">
			...
			<!-- form start -->
					<div class="box-body">
						...
						<div class="form-group" id="group_att">
							<label class="col-sm-3 control-label">Dokumen Skema</label>
							<div class="col-sm-9">
								<input onchange="ValidateSize(this)" type="file" class="form-control pull-right" name="komp[]" id="komponen1"/>
								<label style="color:#FF0000; font-size:10px;">Max Size 5 MB</label>
							</div>
						</div> 
						...
						<div class="form-group" id="group_att1">
							...
							<div class="col-sm-9">
								<input onchange="ValidateSize(this)" type="file" class="form-control pull-right" name="komp[]" id="komponen2"/>
								<label style="color:#FF0000; font-size:10px;">Max Size 5 MB</label>
							</div><!-- /.input group -->
						</div><!-- /.form group -->
