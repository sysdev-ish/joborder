	...
	if(commentReject.length<1)
	{
		...
		if(confirm("Are you sure want to reject this data?"))
		{
				...
				$.ajax(
					 {
							 type: 'POST',
							 url: '<?php echo base_url(); ?>index.php/JobOrder/joTrainingRecruitmentReject',
							 dataType: 'json',
...
function process()
{
	if(confirm("Are you sure want to process this data?"))
	{
			...
			$.ajax(
				 {
						 type: 'POST',
						 url: '<?php echo base_url(); ?>index.php/JobOrder/jobTrainingRecruitmentApprovalApprove',
						 dataType: 'json',
						 data: {'jono': jonoStr},
						 beforeSend: function (jqXHR, settings) {
						 },
...
function getList()
{
		...
		$.ajax(
			 {
					 type: 'POST',
					 url: '<?php echo base_url(); ?>index.php/JobOrder/JSjobTrainingRecruitmentApproval',
					 dataType: 'json',
...
function listDetail(datas)
{
		...
		$.ajax(
			 {
					 type: 'POST',
					 url: '<?php echo base_url(); ?>index.php/JobOrder/JobOrderTrainingDetailRecruitmentApproval',
					 dataType: 'json',
					 data: {'jonos': jonoStr },
					 beforeSend: function (jqXHR, settings) {
					 },
				 ...
				 <div class="modal-content">
					...
					<div class="modal-footer">
						...
						<button type="button" title="Reject Data"  class="btn btn-warning" id="addrincian" onclick="reject()"> <i class="fa fa-arrow-circle-left"></i> Reject Data  </button>
						  ...
						  <button type="submit" title="Approve Data"  class="btn btn-success" id="btnProc" onclick="process()"> Approve Data <i class="fa fa-arrow-circle-right"></i> </button>
					</div>
