...
<script type="text/javascript">
	$(function() {
		...
		$('#cartglcreat').datepicker({
			format: "mm-yyyy",
			startView: "months",
			minViewMode: "months",
			autoclose: true
		});
		$('#cartglbek').datepicker({
			format: "mm-yyyy",
			startView: "months",
			minViewMode: "months",
			autoclose: true
		...
		*/
		...
		$("#btn_excel").click(function() {
			...
			if (tcr1 == '' || tcr2 == '') {
				alert("Range tanggal create harus di isi");
				return false;
			}
			// alert(carpm);
		...
		$("#done_slot").click(function() {
			$('#overlay').show();
			var ket_slot = $('#ket_slot').val();
			var id_slot = $('#id_slot').val();
			var nojo_slot = $('#nojo_slot').val();
			var typejo_slot = $('#typejo_slot').val();
			if (ket_slot == '') {
				$('#overlay').hide();
				alert("Keterangan PM harus di isi, TerimaKasih");
				return false;
			}
			var url = "<?php echo base_url(); ?>index.php/home/doneslot_pm";
			arrappjo = [id_slot, nojo_slot, typejo_slot, ket_slot];
			$.post(url, {
				arrappjo: arrappjo
			}, function(res) {
				$('#overlay').hide();
		...
		$("#rbtn_reject").click(function() {
			...
			arrappjo = [vnojo, nid, rket, tjok, namaz, rpilpks, flagstat];
			$.post(url, {
				arrappjo: arrappjo
			}, function(res) {
				$('#overlay').hide();
		...
		$("#btn_simpan_cancel_sk").click(function() {
			if ($("#scancelz").val() == "") {
				alert("Keterangan Cancel tidak boleh kosong");
				return false;
			}
...
<script type="text/javascript">
	...
	function badd(xkid, xnojo, xkdone, xkpro, xknpro, xknlok, xklok, btn, zparam, xjabatan, xskill, ketcan, tyjo, cuk, ntype, upd, alev, lev_sap, persa_sap, skill_sap, area_sap, jab_sap, skema_sap, abkrs_sap, jpro_sap, kontrakh, dekomp, jnew, new_trep, pernernorek, nreason, sestype, username, cpilpks, cdokpks, cdivisiid, dlegalitas) {
		// alert(dlegalitas);
		...
		$("#rpilpksss").val(dlegalitas);
		$("#npilpksss").val(dlegalitas);
		...
		if (btn == "Done" || btn == "Stop Fulfilled" || btn == "Stop Not Fulfilled") {
			...
			$("#btnstop").click(function() {
				if ($("#osstop").val() == "") {
					alert("Keterangan Stop Pemenuhan tidak boleh kosong");
					return false;
				}
				var vid = $('#det_id').val();
		...
		} else if (btn == "Canceled" || btn == "Return") {
			$('#divstop').hide();
			$('#polo').show();
			if (tyjo == 2) {
				$("#rket").val(ketcan);
			...
			if (jnew == 'INF') {
				var appbtn1 = '<button type="button" class="btn btn-outline" data-dismiss="modal" id="btnreject1">Reject</button><button type="button" class="btn btn-outline" data-dismiss="modal" id="btnreturn1">Return</button><button type="button" class="btn btn-outline" data-dismiss="modal" id="btnsimpan2">Go Rekrut</button>';
			} else {
				var appbtn1 = '<button type="button" class="btn btn-outline" data-dismiss="modal" id="btnreject1">Reject</button><button type="button" class="btn btn-outline" data-dismiss="modal" id="btnreturn1">Return</button><button type="button" class="btn btn-outline" data-dismiss="modal" id="btnsimpan1">Done</button>';
			}
			...
			$("#btnreject1").click(function() {
				if ($("#oscancel").val() == "") {
					alert("Keterangan Reject tidak boleh kosong");
					return false;
				}
				var vnojo = $('#det_nojo').val();
				var scancel = $('#oscancel').val();
				...
				arrappjo = [vnojo, scancel, vid, npilpks, flagstat];
				$.post(url, {
					data: arrappjo
				}, function(res) {
					alert('Data Rejected');
			...
			$("#btnreturn1").click(function() {
				if ($("#oscancel").val() == "") {
					alert("Keterangan Return tidak boleh kosong");
					return false;
				}
				var vnojo = $('#det_nojo').val();
				var scancel = $('#oscancel').val();
		...
		if (sestype == 'PM') {
			document.getElementById('rbtn_simpan').setAttribute("style", "display:none;");
			document.getElementById('rbtn_reject').setAttribute("style", "display:none;");
			document.getElementById('rbtn_return').setAttribute("style", "display:none;");
			document.getElementById('approvalbtn').setAttribute("style", "display:none;");
			document.getElementById("ruspm").value = username;
			// $("input[name=rpilpks][value='" + cpilpks + "']").prop('checked', true);
			// document.getElementById("rdokpks").value = cdokpks;
			document.getElementById("uspm").value = username;
			document.getElementById("udiv").value = cdivisiid;
			// $("input[name=npilpks][value='" + cpilpks + "']").prop('checked', true);
			// document.getElementById("ndokpks").value = cdokpks;
			...
			$("#jenisjabatan").val(jab_sap).trigger('change');
			$("#jenislevel").val(lev_sap).trigger('change');
			document.getElementById("jenisproject").value = jpro_sap;
			$("#zskema").val(skema_sap).trigger('change');
		} else {
			// document.getElementById('rbtn_simpan').removeAttribute("style");
			// document.getElementById('rbtn_reject').removeAttribute("style");
			document.getElementById('approvalbtn').removeAttribute("style");
			document.getElementById("uspm").value = username;
			document.getElementById("udiv").value = cdivisiid;
			// $("input[name=npilpks][value='" + cpilpks + "']").prop('checked', true);
			// document.getElementById("ndokpks").value = cdokpks;
	...
	function actpm(id, nojo, ketpm, typejo, btnpm) {
		// alert(id);
		$('#nojo_slot').val(nojo);
		$('#id_slot').val(id);
		$('#typejo_slot').val(typejo);
	...
	function ceket_pm(ket_done_pm) {
		$('#desketpm').val(ket_done_pm);
	}
</script>
		...
		<div class="box box-default" id="tar1">
			<div class="box-header with-border">
				<div class="row">
					...
					<div class="col-sm-1"><label>&nbsp;</label>
						<center>sd</center>
					</div>
					<div class="col-sm-2">
						<label>Tanggal Create</label>
					...
					<div class="col-sm-3">
						<button type="button" class="btn btn-primary" id="btn_excel" style="margin-top:25px;">Export xlsx</button>
				...
				<div class="box-body">
					<table id="listorder" class="table table-bordered table-hover">
						<thead style="background-color: #dd4b39; color:white;">
							...
							<tr>
								<!--<th><select name="carnojo" id="carnojo" class="form-control select2 pull-right" style="width:100%"><option value="">Cari Nojo</option>	<?php //echo $cmbnojo;
																																																																														?></select></th>-->
								...
								<th align="center"></th>
								<th align="center"><select name="carhire" id="carhire" class="form-control select2 pull-right" style="width:100%">
										<option value=""></option> <?php echo $cmbjmlx; ?>
									</select></th>
								<th align="center"></th>
								...
								<th></th>
								<th><select class="form-control select2 pull-right" id="carpm" name="carpm" style="width:100px;">
										<option value=""></option><?php echo $cmbpm; ?>
									</select></th>
								<th></th>
								<th></th>
								<?php if ($level == 5 and ($type == 'PPC' or $type == 'AVP' or $type == 'VP')) { ?>
									<th><select name="cardone" id="cardone" class="form-control select2 pull-right" style="width:80px">
											<option value=""></option>
											<option value="1">DONE</option>
											<option value="2">OnProgress</option>
											<option value="3">Reject</option>
										</select></th>
								<?php } else if ($level == 5 and $type == 'PM') { ?>
									<th><select name="cardone" id="cardone" class="form-control select2 pull-right" style="width:80px">
											<option value=""></option>
											<option value="1">DONE</option>
											<option value="2">OnProgress</option>
				...
				<div class="modal-dialog" id="modal-alert" style="width:980px;">
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div class="form-group col-md-12" id="divrpks">
								...
								<div class=" ">
									...
									<input type="text" class="form-control" id="rpilpksss" name="rpilpksss" readonly>
								</div><!-- /.input group -->
							</div><!-- /.form group -->
							<br><br><br>
						...
						<div class="modal-footer" style="background-color:blue; color:white;">
							...
							<button type="button" class="btn btn-outline" data-dismiss="modal" id="rbtn_return">Return</button>
											...
											<div class="col-xs-12" style="width:100%;height:100%;border:1px solid #000; margin-bottom:20px;" id="koko">
												<?php foreach ($rdoc as $list) { ?>
													<div class="col-md-4" style="margin-top:10px;">
														<input type="checkbox" <?php if (strpos($zit, $list['doc_id']) !== false) {
																											echo 'checked';
															...
															<div class="">
																<select class="form-control select2 pull-right" id="jenispersa" name="jenispersa" style="width:100%;">
																	...
																	foreach ($cmbpersax as $key => $list) { ?>
																		<option value="<?php echo $list->kd_persa; ?>"><?php echo $list->persa . ' (' . $list->kd_persa . ')'; ?></option>
																	<?php } ?>
																</select>
															</div><!-- /.input group -->
														</div><!-- /.form group -->
															...
															<div class="">
																<select class="form-control select2 pull-right" id="jenispay" name="jenispay" style="width:100%;">
																	...
																	<?php foreach ($cmbabkrsx as $key => $list) { ?>
																		<option value="<?php echo $list->PAYROLL_AREA; ?>"><?php echo $list->PAYROLL_AREA_TEXT . ' (' . $list->PAYROLL_AREA . ')'; ?></option>
																	<?php } ?>
																</select>
															</div><!-- /.input group -->
														</div><!-- /.form group -->
															...
															<div class="">
																<select class="form-control select2 pull-right" id="jenisskill" name="jenisskill" style="width:100%;">
																	...
																	<?php foreach ($cmbskill as $key => $list) { ?>
																		<option value="<?php echo $list->PERSK; ?>"><?php echo $list->PTEXT . ' (' . $list->PERSK . ')'; ?></option>
																	<?php } ?>
																</select>
															</div><!-- /.input group -->
														</div><!-- /.form group -->
															...
															<div class="">
																<select class="form-control select2 pull-right" id="det_area" name="det_area" style="width:100%;">
																	...
																	<?php foreach ($cmbarea as $key => $list) { ?>
																		<option value="<?php echo $list->AREA; ?>"><?php echo $list->AREA_TEXT . ' (' . $list->AREA . ')'; ?></option>
																	<?php } ?>
																</select>
															...
															<div class="">
																<select class="form-control pull-right select2" id="jenisjabatan" name="jenisjabatan" style="width:100%;">
																	...
																	<?php foreach ($cmbjabatan as $key => $list) { ?>
																		<!-- <option value="<?//php echo $list->OBJID; ?>"><?//php echo $list->STEXT . ' (' . $list->OBJID . ')'; ?></option> -->
															...
															<div class="">
																<select class="form-control pull-right select2" id="jenislevel" name="jenislevel" style="width:100%;">
																	...
																	<?php foreach ($cmblevel as $key => $list) { ?>
																		<option value="<?php echo $list->TRFAR; ?>"><?php echo $list->TARTX . ' (' . $list->TRFAR . ')'; ?></option>
																	<?php } ?>
																</select>
															</div><!-- /.input group -->
														</div><!-- /.form group -->
										...
										<div class=" ">
											<!-- <div class="col-sm-12"> -->
												...
												<div class="col-sm-6">
													...
													<input type="text" class="form-control" id="npilpksss" name="npilpksss" readonly>
											<!-- </div> -->
										</div><!-- /.input group -->
									</div><!-- /.form group -->
						...
						<div class="modal-body">
							<form class="">
								<div class="box-body">
									...
									<div>
										<label>
											<center>Data Perner Rotasi</center>
										</label>
									</div>
									...
									<div id="datares">
										<div>
											<label>
												<center>Data Perner Resign</center>
											</label>
										</div>
			...
			<div class="modal fade" id="modalpm" role="dialog">
				<div class="modal-dialog" id="modal-alert" style="width:980px;">
					<div class="modal-content">
						<div class="modal-header" style="background-color:blue; color:white;">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
							<h4 class="modal-title">Done Slot</h4>
						</div>
						<div class="modal-body">
							<textarea class="form-control" rows="3" id="ket_slot" name="ket_slot" placeholder="Keterangan PM..." /></textarea>
							<br>
							<input type="hidden" id="nojo_slot" name="nojo_slot">
							<input type="hidden" id="id_slot" name="id_slot">
							<input type="hidden" id="typejo_slot" name="typejo_slot">
						</div>
						<div class="modal-footer" style="background-color:blue; color:white;">
							<!--<button type="button" class="btn btn-outline" data-dismiss="modal" id="btn_close">Close</button>-->
							<button type="button" class="btn btn-outline" data-dismiss="modal" id="btn_close">Close</button>
							<button type="button" class="btn btn-outline" id="done_slot">Done</button>
						</div>
					</div><!-- /.modal-content -->
				</div><!-- /.modal-dialog -->
			</div><!-- /.example-modal -->
			...
			<div class="modal fade" id="Modal_cekpm" role="dialog">
				<div class="modal-dialog" id="modal-alert" style="width:500px;">
					<div class="modal-content">
						<div class="modal-header" style="background-color:blue; color:white;">
							<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
							<h4 class="modal-title">Keterangan PM</h4>
						</div>
						<div class="modal-body">
							<form class="">
								<div class="box-body">
									<textarea id="desketpm" cols="55" rows="10"></textarea>
								</div><!-- /.box-body -->
							</form><!-- /.form-horizontal -->
						</div>
						<div class="modal-footer" style="background-color:blue; color:white;">
