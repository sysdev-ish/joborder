	...
	public function listorderxy()
	{
		if ($this->session->userdata('logged_in')){
			...
			$typ 		= $session_data['type'];
				...
				$this->load->view('pages/header',$data);
				$this->load->view('joborder/listorder_sap_blmujicoba',$data);
				$this->load->view('pages/footer');
	...
	*/
	...
	function cek_mandat(){
		if ($this->session->userdata('logged_in')){
			...
			$def = $this->db->query("SELECT GROUP_CONCAT(kode) AS mandat FROM komponen WHERE mandatory='1' AND status_kontrak='$kontzx' ")->row();
	...
	function m_simpantjo_rasap(){
		if ($this->session->userdata('logged_in')){
			...
			$noob = $this->db->query("Select tanggal, lup_edit From trans_jo Where nojo='$njok' ")->row();
			...
			if($aws <= 0){
				...
				$awsk = $this->db->query("SELECT area FROM trans_perner Where id='$nid' ")->row();
				...
				$awsk = $this->db->query("SELECT lokasi FROM trans_rincian Where id='$nid' ")->row();
	...
	function m_rejectjo_rrsap(){
		if ($this->session->userdata('logged_in')){
			...
			$namz  	= $njo[4];
			$pilpks = $njo[5];
			$this->job_model->m_rejectjo_rrsap($njok, $keter, $pilpks);
			$this->job_model->m_rejectjo_rrsap2($nid, $keter, $pilpks);
		...
		if ($this->session->userdata('logged_in')){
			...
			*/
			...
			foreach ($awskg as $val) {
				...
				if($val['layanan']=='L018'){
					$hasilkirim = $this->emailsent_sap($test,'recruitment.jabar@ish.co.id','Notifikasi Done Skema JO',$message);
				} else {
					$hasilkirim = $this->emailsent($test,'soehartobaru@gmail.com','Notifikasi Pengajuan JO',$message);
				}
			}
	...
	*/
	...
	function rincian_simpan(){
		...
		*/
		...
		$loop = count($rec)/17;
		if ($loop){
			$a = 0; $b = 1; $c = 2; $d = 3; $e = 4; $f = 5; $g = 6; $h = 7; $o = 8; $j = 9; $k = 10; $l = 11; $m = 12; $n = 13; $p = 14; $q = 15;
			for ($i=0; $i<$loop; $i++){
				$job_data['company_id'] = '97';
	...
	*/
		...
		foreach($_POST['choosen'] as $key => $list){
			...
			// {
				...
				if(empty($check)){
					...
					$hasilkirim = $this->emailsent($tust,'soehartobaru@gmail.com','Notifikasi Pengajuan JO - Penyesuaian Skema',$message);
				}
				else {
					$test = array();
					foreach ($check as $val) {
	...
	function s_edit_simpan_allx(){
		...
		if($rec!='X')
		{
			...
			$loopx = $loop-1;
			if ($loopx){
				$a = 0; $b = 1; $c = 2; $d = 3; $e = 4; $f = 5; $g = 6; $h = 7; $o = 8; $j = 9; $k = 10; $l = 11; $m = 12; $n = 13; $p = 14; $q = 15; $r = 16; $s = 17; $t = 18; $u = 19; $v = 20; $w = 21; $x = 22; $y = 23;
				for ($i=0; $i<$loopx; $i++){
	...
	function s_perner_simpanx(){
		...
		if($rec!='')
		{
			...
			$loopx = $loop-1;
			if ($loopx){
				$a = 0; $b = 1; $c = 2; $d = 3; $e = 4; $f = 5; $g = 6; $h = 7; $o = 8; $j = 9; $k = 10; $l = 11; $m = 12; $n = 13; $p = 14; $q = 15; $r = 16; $s = 17; $t = 18; $u = 19; $v = 20; $w = 21; $x = 22;  $y = 23; $z = 24; $az = 25;
				for ($i=0; $i<$loopx; $i++){
	...
	function s_perner_simpan(){
		...
		*/
		...
		if($rec!='')
		{
			...
			$loopx = $loop-1;
			if ($loopx){
				$a = 0; $b = 1; $c = 2; $d = 3; $e = 4; $f = 5; $g = 6; $h = 7; $o = 8; $j = 9; $k = 10; $l = 11; $m = 12; $n = 13; $p = 14;
				for ($i=0; $i<$loopx; $i++){
					if (strpos($rec[$m], '1') !== false) {
						$ca = 1;
					} else {
	...
	function s_sk(){
		if ($this->session->userdata('logged_in')){
			...
			$hasilkirim = $this->emailsent_rej($test,'soehartobaru@gmail.com','Notifikasi Pengajuan JO - Penyesuaian Skema',$message);
	...
	function simpan_skema_sk(){
		if ($this->session->userdata('logged_in')){
			...
			$hasilkirim = $this->emailsent_sap($test,$bwk->email,'Notifikasi Done Skema JO - Penyesuaian Skema',$message);
		}
	}
	...
	function perner_search(){
		...
		$data	= $this->skema_model->perner_search($perner);
	...
	function report_excel(){
		...
		if ($this->session->userdata('logged_in')){
			$session_data 			= $this->session->userdata('logged_in');
			$data['allrep']		= $this->job_model->report_excel($tgc1, $tgc2, $pm, $stat, $cnobak,$cpro,$clok,$ccre,$cnoj,$data['username'],$data['level'],$data['regional']);
