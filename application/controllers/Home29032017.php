...
class Home extends CI_Controller {
	...
	*/
	...
	function simpantjo(){
		if ($this->session->userdata('logged_in')){
			...
			$keter = $njo[1];
			$tjo   = $njo[2];
			$this->job_model->inssimpantjo($njok,$keter, $tjo);
	...
	*/
	...
	function urincian_simpan(){
		...
		*/
		...
		for($i=0; $i<count($_FILES['komponen']['name']); $i++){
			$target_path = "./uploads/";
			$ext = explode('.', basename( $_FILES['komponen']['name'][$i]));
			//$target_path = $target_path . md5(uniqid()) . "." . $ext[count($ext)-1]; 
			$target_path = $target_path . $hjk[$i] ."_". $nojo . "." . $ext[count($ext)-1]; 

			if(move_uploaded_file($_FILES['komponen']['tmp_name'][$i], $target_path)) {
	...
	function rincian_simpan(){
		...
		$daud = $this->input->post('joborder[8]');
			...
			$this->job_model->campuran($nojof, $file_name);	
	...
	*/
	...
	function urincian_simpanx(){
		...
		*/
		...
		for($i=0; $i<count($_FILES['file']['name']); $i++){
			...
			//$target_path = $target_path . md5(uniqid()) . "." . $ext[count($ext)-1]; 
			$target_path = $target_path . $hjk[$i] ."_". $sare ."_". $spa ."_". $date_now . "." . $ext[count($ext)-1]; 
	...
	function rincian_simpanx(){
		...
		$file_element_name  = $this->input->post('joborder[2]');	
		$file_element_name2 = $this->input->post('joborder[3]');	
		$file_element_name3 = $this->input->post('joborder[4]');	
		$sar = $this->input->post('joborder[0]');
		$spa = $this->input->post('joborder[1]');
		$ext  = pathinfo($file_element_name, PATHINFO_EXTENSION);
		...
		{
			$file_name  = "skema_" . $sar . "_" . $spa . "_" . $date_now . "." . $ext;
		}
		...
		{
			$file_name2 = "bak_" . $sar . "_" . $spa . "_" . $date_now . "." . $ext2;
		}
		...
		{
			$file_name3 = "other_" . $sar . "_" . $spa . "_" . $date_now . "." . $ext3;
		}
