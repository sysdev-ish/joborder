...
class Mapping extends CI_Controller {
	...
	public function cek_perner_ganti()
	{
		if ($this->session->userdata('logged_in')){
			...
			$cektglakt = $this->skema_model->cektglakt($pergan); 
	...
	public function cek_perner_aktif()
	{
		if ($this->session->userdata('logged_in')){
			...
			$cekok = $this->db->query("SELECT a.perner, a.nojo, r.status_rekrut FROM trans_perner a left join trans_jo b ON a.nojo=b.nojo LEFT JOIN (SELECT id, idpktable, status_rekrut, jumlah FROM trans_rincian_rekrut WHERE typejo='2') r ON r.idpktable=a.id WHERE a.perner='$perner' AND b.flag_cancel_sap!='1' AND b.approval!='2' AND r.status_rekrut NOT IN (3,4) AND a.persa='$persax_sap' AND a.area='$areax_sap' AND a.skilllayanan='$skillx_sap' AND a.level='$lvlx_sap' AND a.nm_jabatan='$jbtnx_sap' AND (a.skema!='1' OR a.flag_app!='2') ORDER BY a.id DESC LIMIT 1 ");
				...
				*/
				...
				if($row['type_jo']==1){
					...
					if($row['rotasi_resign']!=''){
						if($row['rotasi_resign']=='2'){$jrr='Rotasi/Mutasi';}elseif($row['rotasi_resign']=='1'){$jrr='Resign';}else{$jrr='Data Lama';}
						if($row['type_replace']==1){
	...
	*/
	...
	public function data_listorder_sap()
	{
		...
		if (isset($session)){
			...
			$data = $this->job_model->get_transall_sap_2($length,$start,$search,$order,$dir,$parsearch);
	...
	*/
	...
	public function donlot_dokperal()
	{
		...
		$caripro  = $_GET['caripr'];
		$carinbak = $_GET['carinbak'];
		$data['listjo'] 	= $this->job_model->dok_peralihan($caripro);
