...
class Job_model extends CI_Model
{
	...
	*/
	...
	function get_transappjo2($length, $start, $search, $order, $dir)
	{
		...
		// else {
		// 	$where_clause .= " AND a.approval IN (1, 2) OR (a.approval IN (0,1,2) AND (YEAR(a.tanggal) >= " . $yearnow . ") ) ";
		// }
	...
	function get_transappjo_enterprise($length, $start, $search, $order, $dir)
	{
		...
		$query .= "LEFT JOIN m_login b ON a.`upd` = b.`username` ";
		$query .= "left JOIN m_type c ON a.`type_jo` = c.`id` ";
		//change by kaha 05/04/2022
		$query .= "WHERE a.kode_cust IN ($cekid_client->kumpid) and b.jabatan='J003' AND b.layanan='L021'  ";
		// $query .= "WHERE a.kode_cust IN ($cekid_client->kumpid) and b.jabatan='$jabatan' AND b.layanan='$layanan'  ";
		$query .= "$where_clause ";
		$query .= "order by a.approval ASC, a.nojo DESC ";
		$Q		= $this->db->query($query . " LIMIT $start, $length");
	...
	function get_transappjo($length, $start, $search, $order, $dir)
	{
		...
		// else {
		// 	$where_clause .= " AND (a.approval IN (0,1,2) AND (YEAR(a.tanggal) >= " . $yearnow . ") ) ";
		// }
		//$data			= array();
		$query = "SELECT a.nojo, a.tanggal, a.kode_cust, a.project, a.syarat, a.deskripsi, a.bekerja, a.upd, a.lup, a.approval, a.approval_admin, a.komponen, ket_atasan, ket_admin, a.type_jo, a.type_replace, b.nama as nupd, a.type_new, a.n_project, c.nama as ntype_jo, (SELECT doc_id FROM trans_doc WHERE trans_doc.`nojo`=a.`nojo`) AS doc_id, a.flag_cancel_sap FROM trans_jo a ";
		//$query .= "LEFT JOIN m_login b ON a.`upd` = b.`nama` ";
	...
	{
		$data			= array();
		$query = "SELECT a.nojo, a.n_area, a.n_perar, a.area, a.perar, a.dokumen_skema as skema, a.dokumen_bak as bak, a.dokumen_other as other, b.nama, a.lup, a.flag_approval, a.ket_reject, a.flag_skema, a.ket_done FROM trans_skema a ";
		$query .= "LEFT JOIN m_login b ON a.`upd` = b.`username` ";
	...
	{
		$data			= array();
		$query = "SELECT a.nojo, a.n_area, a.n_perar, a.area, a.perar, a.dokumen_skema as skema, a.dokumen_bak as bak, a.dokumen_other as other, b.nama, a.lup, a.flag_approval, a.ket_reject, a.ket_done FROM trans_skema a ";
		$query .= "LEFT JOIN m_login b ON a.`upd` = b.`username` ";
	...
	*/
	...
	function newget_transall_sap_2($length, $start, $search, $order, $dir, $parsearch)
	{
		...
		$query .= "GROUP BY a.id ";
		$query .= "UNION ALL ";
		$query .= "SELECT REPLACE(b.komponen, ' ', '_') AS komponen, a.id, a.`nojo`, '' AS komentar, a.skema, b.`project`, d.level, b.`tanggal`, b.latihan, '' AS `jabatan`, '' AS gender, '1' AS `jumlah`, a.nm_area AS lokasix, b.bekerja, '' AS atasan, a.nm_area as lokasi, '' AS name_job_function, b.ket_cancel, b.flag_cancel, b.flag_cancel_sap, a.flag_app, '' AS ket_rej, b.type_jo, d.nama as dnama, '' AS status_rekrut, '' AS ket_rekrut, g.nama AS ntype, '' AS pic_hi, a.nm_persa as n_project, b.skema AS skemax, '' AS pendidikan, a.perner, a.nama AS anama, a.area, a.nm_area, b.nojo AS bnojo, b.deskripsi, a.ket_done as bket_done, '' as skilllayanan, b.type_replace, '' as kontrak, '' as waktu, a.lup, b.ket_atasan, b.ket_admin, b.ket_done, b.type_new, a.nm_persa, b.no_bak, '' as alev, '' as level_sap, '' as persa_sap, '' as skill_sap, '' as area_sap, '' as jabatan_sap, '' as skema_sap, '' as abkrs_sap, '' as jenis_pro_sap, '' as zparam, '' as detail_komp, a.lup_skema, b.lup_cancel_skema, b.type_replace as rekrutx, b.lup as blup, r.id AS id_hire, p.jml as jml_hire, a.userpm, (select nama from m_login where m_login.username=a.userpm) as nuserpm, a.rotasi_resign, b.lama, b.lup_edit, b.lupapp_atasan, a.lup_newrej, a.lup_edit as alup_edit, a.kunci_tgl_create, r.status_rekrut as rstatus_rekrut, r.jumlah as jml_stop, b.flag_peralihan, b.bekerja_edit, a.type_rep as type_rekrut, a.tglbekerja as tgl_bekerja, a.tgllatihan as tgl_latihan, '' as perner_norekrut, '' as nreason, b.nama_cust, a.perner_ganti, a.flagrfc as status_pernernewjo, a.flag_done_pm, a.ket_done_pm, b.pilpks, b.dokpks, b.divisiid, b.pks as legalitas,  (select flagrfc from trans_perner_ganti Where nojo=a.nojo and perner_resrot=a.perner and perner_ganti=a.perner_ganti order by id asc limit 1) as rfc_rotasi ";
		// $query .= "'' AS rekrut, '' AS hr, '' AS jmluser, '' AS training, '' AS note, (SELECT nama FROM m_login WHERE m_login.username=b.upd) AS upd   ";
		$query .= "FROM trans_perner a  ";
		// $query .= "LEFT JOIN mapping_city e ON a.`area` = e.`city_id`  "; 
		$query .= "LEFT JOIN saparea e ON a.`area` = e.value1  ";
		...
		$query .= ") AS u ";
		$query .= "WHERE u.bnojo!='' ";
		// $query .= "AND date_format(u.tanggal,'%m-%Y') > '01-".$yearstart."' ";
		// $query .= " AND (MONTH(u.tanggal) between '" . $startmonth . "' and '" . $endmonth . "') AND YEAR(u.tanggal) = '" . $yearnow . "' ";
		$query .= "$where_clause ";
		if ($cpro != '') {
			$query .= "AND (u.n_project like '%" . $cpro . "%' OR u.project like '%" . $cpro . "%') ";
		}
		...
		if ($ctglcr != '') {
			...
			$query .= " AND (MONTH(u.tanggal) between '" . $startmonth . "' and '" . $endmonth . "') AND (YEAR(u.tanggal) between  " . $yearstart . " and " . $yearnow . ") ";
		}
		if ($ctglbk != '') {
			// $query .= "AND date_format(u.bekerja,'%m-%Y') = '" . $ctglbk . "' ";
			$query .= "AND date_format(u.tgl_bekerja,'%m-%Y') = '" . $ctglbk . "' ";
		}
		$query .= "HAVING id IS NOT NULL ";
		if ($chire != '') {
			if ($chire == 0) {
		...
		// $query .= "ORDER BY (u.skema=0 OR u.skema IS NULL OR u.skema='') DESC, (u.flag_cancel_sap=0 OR u.flag_cancel_sap IS NULL OR u.flag_cancel_sap='') DESC, u.tanggal DESC  ";
		// $query .= "ORDER BY u.skema ASC, u.nojo DESC";
		$query .= "ORDER BY (u.skema=0 OR u.skema IS NULL OR u.skema='') DESC, (u.flag_cancel_sap=0 OR u.flag_cancel_sap IS NULL OR u.flag_cancel_sap='') DESC, u.nojo DESC  ";
	...
	*/
	...
	public function take_lok_local()
	{
		$data = array();
		//$dbpostgre = $this->load->database('db_second',TRUE);
		$query	= "select a.city_id, a.city_name from mapping_city a where city_id NOT IN('I595','I652') order by a.city_name ";
		$Q		= $this->db->query($query);
		if ($Q->num_rows() > 0) {
			foreach ($Q->result_array() as $row) {
				$data[] = $row;
	...
	public function take_kat($kode_jabatan)
	{
		$data = array();
		//$dbpostgre = $this->load->database('db_second',TRUE);
		$query	= "SELECT a.id, a.name_job_function, b.name_job_function_category
				FROM job_function a
				LEFT JOIN job_function_category b ON b.id=a.function_category
				where a.id='$kode_jabatan'
				ORDER BY name_job_function";
	...
	public function get_detailkategori()
	{
		$data = array();
		//$dbpostgre = $this->load->database('db_second',TRUE);
		$query	= "SELECT a.id, a.name_job_function, b.name_job_function_category
	...
	function xinssimpanskemax($vid, $scancel, $mymy, $det_area, $jenisskill, $jenisjabatan, $jenislevel, $jenisproject, $zskema, $jpay, $ubab, $zparam, $username, $tglinp, $njab, $uspm, $knci, $jkontrak, $trfgb, $alasanrot, $udiv, $pilpks, $dokpks)
	{
		...
		$update	= array(
			...
			'skema_sap' 		=> $zskema,
			'abkrs_sap' 		=> $jpay,
			// 'hire_jabatan_sap' => $jenisjabatan,
			'hire_jabatan_sap' 	=> $ubab, //changebykaha 5/10/22
			// 'cttyp_sap' 		=> $jkontrak,
			// 'trfgb_sap' 		=> $trfgb,
			// 'kd_rotasi' 		=> $rec[1],
			// 'kd_reason' 		=> $rec[0],
	...
	function get_topod()
	{
		...
		$query .= "WHERE c.tanggal LIKE '%2020%' AND (c.approval=1 OR c.approval=5) AND DATEDIFF(c.latihan,CURRENT_DATE()) < '0' AND c.flag_cancel_sap!=1 ";
		$query .= "GROUP BY b.id  ";
...
*/
	...
	*/
	...
	function get_listorder1($length, $start, $search, $order, $dir)
	{
		...
		// else {
		// 	$where_clause .= " AND (YEAR(u.tanggal) >= " . $yearnow . ") ";
		// }
		...
		$query .= "$where_clause ";
		$query .= "ORDER BY u.nojo desc ";
		// $query .= "ORDER BY b.approval ASC ,u.nojo DESC ";
	...
	*/
	...
	{
		$data			= array();
		$query = "SELECT a.id, a.nojo, a.n_area, a.n_perar, a.area, a.perar, a.dokumen_skema as skema, a.dokumen_bak as bak, a.dokumen_other as other, b.nama, a.lup, a.flag_approval, a.ket_reject, a.flag_skema, a.flag_cancel_sap, a.ket_cancel, a.flag_manar, a.ket_manar, a.ket_done FROM trans_skema a ";
		$query .= "LEFT JOIN m_login b ON a.`upd` = b.`username` ";
	...
	function get_transall_sap_sk_manar($search, $username)
	{
		...
		$query = "SELECT a.id, a.nojo, a.n_area, a.n_perar, a.area, a.perar, a.dokumen_skema as skema, a.dokumen_bak as bak, a.dokumen_other as other, (select nama from m_login where m_login.username=a.upd) as nama, a.lup, a.flag_approval, a.ket_reject, a.flag_skema, a.flag_cancel_sap, a.ket_cancel, a.flag_manar, a.ket_manar, a.ket_done FROM trans_skema a ";
		$query .= "LEFT JOIN mapping_city e ON a.`area` = e.`city_id` ";
		$query .= "LEFT JOIN m_login b ON e.`manar` = b.`username` ";
	...
	function get_transall_sap_sk_manar2($length, $start, $search, $order, $dir)
	{
		...
		$query = "SELECT a.id, a.nojo, a.n_area, a.n_perar, a.area, a.perar, a.dokumen_skema as skema, a.dokumen_bak as bak, a.dokumen_other as other, (select nama from m_login where m_login.username=a.upd) as nama, a.lup, a.flag_approval, a.ket_reject, a.flag_skema, a.flag_cancel_sap, a.ket_cancel, a.flag_manar, a.ket_done FROM trans_skema a ";
		$query .= "LEFT JOIN mapping_city e ON a.`area` = e.`city_id` ";
		$query .= "LEFT JOIN m_login b ON e.`manar` = b.`username` ";
		$query .= "WHERE a.flag_approval = '1' and e.manar = '$username' ";
		$query .= "$where_clause ";
	...
	*/
	...
	function report_excel($tgc1, $tgc2, $pm, $stat, $cnobak, $cpro, $clok, $ccre, $cnoj, $username, $level, $reg, $layanan, $jabatan)
	{
		$data			= array();
		...
		$query .= "u.lama, u.jumlah, u.skema, u.approval, u.jenis_captive, u.lup_skema, u.flag_cancel_sap, u.skemax, u.flag_cancel, u.jabatan, u.jabatanx, u.id_hire, u.jml_hire, u.userpm, u.nuserpm, u.bekerja, u.zparam, u.lup_edit, u.lupapp_atasan, u.lup_newrej, u.alup_edit, u.kunci_tgl_create, u.fstatus_rekrut, u.jml_stop, u.id, u.atrek, u.bekerja_rincian, u.perner_ganti, u.status_pernernewjo, u.rfc_rotasi, u.jml_candidate, u.remarks, u.uak  ";
		$query .= "From ( ";
	...
	function save_transpks($nojo, $vid, $jns, $username, $userpm)
	{
		...
		if($cekdet->pks > 0){
			$cek_data_legal_pks	= $this->db->query("select * from legal_pks Where nobak='" . $cekdet->no_bak . "' ")->row();

			$pembuat = $this->db->query("select * from m_login where username = '$cekdet->upd' ")->row();

			if($cekdet->pks == 1){
				$legalitas = 'PKS';
			}else if($cekdet->pks == 2){
				$legalitas = 'Amandemen';
			}else if($cekdet->pks == 3){
				$legalitas = 'Addendum';
			}else if($cekdet->pks == 4){
				$legalitas = 'MOU';
			}else{
				$legalitas = 'Unknown';
			}
			...
			$item_pks = array(
				'nobak' => $cekdet->no_bak,
				'customer' => $cekdet->kode_cust . '||' . $cekdet->nama_cust,
				'project' => $pro,
				'project_start' => $cekdet->start_project,
				'project_end' => $cekdet->end_project,
				'project_long' => $cekdet->lama,
				'project_nilai' => $cekdet->nilai_project,
				'project_type' => $cekdet->type_new,
				'project_divisi' => $cekdet->divisiid,
				'file_bak' => $cekdet->komponen_bak,
				'legalitas_id' => $cekdet->pks,
				'legalitas_name' => $legalitas,
				'project_created' => $cekdet->upd. '-' .$pembuat->nama,
				'created_at' => date("Y-m-d H:i:s"),
				'project_approved_by' => $userid. '-' .$name,
				'project_approved_at' => date("Y-m-d H:i:s")
			);
			
			if (empty($cek_data_legal_pks)) {
				$this->db->insert('legal_pks', $item_pks);
			}
		}
			

		// $this->db->query("update legal_pks set project_status = 1, project_approved_at = '" . date("Y-m-d H:i:s") . "', project_approved_by = '" . $username . "' where  nobak = '" . $cekdet->no_bak . "' ");
	}
